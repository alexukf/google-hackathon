...
public class DiskBasedCache implements Cache {
	...
	private final boolean withClearCache;
	...
	public DiskBasedCache(File rootDirectory, int diskUsageBytes,
			boolean withClearCache) {
		mRootDirectory = rootDirectory;
		mMaxCacheSizeInBytes = diskUsageBytes;
		this.withClearCache = withClearCache;
	}
	 ...
	 * Puts the entry with the specified key into the cache.
	 * 
	 * @return the file path to the storage folder
	 */
	public synchronized String putWithPathReturn(String key, Entry entry) {
		pruneIfNeeded(entry.data.length);
		File file = getFileForKey(key);
		try {
			FileOutputStream fos = new FileOutputStream(file);
	...
	/**
	 * Puts the entry with the specified key and file path into the cache.
	 */
	public synchronized void put(String key, Entry entry, File file) {
		pruneIfNeeded(entry.data.length);
		try {
			FileOutputStream fos = new FileOutputStream(file);
			CacheHeader e = new CacheHeader(key, entry);
